...
from ...module_translation_handler.ver1.module_transformer_handler import TransformersTranslationModelHandler
...
class TransformerFlaggerSeqLogProbability(object):
    def __init__(self, 
    ...
    def _load_cache(self, sentence_id: str, source_text: str) -> _RecordCache:
        ...
        return _RecordCache(sentence_id=cache_obj['sentence_id'], 
    ...
    def compute_dataset_log_probability(self, 
                                        dataset: ty.List[EvaluationTargetTranslationPair]) -> ty.List[float]:
        ...
        for _record in dataset:
            ...
            if path_cache_file.exists():
                ...
                __translated_obj = self.translation_handler.translate_beam_search(
                    input_text=_record.source,
                    temperature=self.temperature_parameter)
                cache_obj = {
                    "sentence_id": _record.sentence_id,
                    "source_sentence": _record.source,
                    "log_probability": __translated_obj.log_score,
                    "translation_text": __translated_obj.translation_text
                }
                torch.save(cache_obj, path_cache_file)
                seq_log_probabilities.append(__translated_obj.log_score)
            # end if
        # end for
    ...
    def get_flag_threshold(self, 
                           ...
                           percentile: float = 40) -> float:
        """I get the threshold for the flagging.
        
        Args:
            seq_log_probability (ty.List[float]): List of log probabilities.
            percentile (float, optional): Percentile. Defaults to 40%, suggested by Guerreiro et al. (2023).
    ...
    def flag(self, 
             source_text: str, 
             threshold: float,
             sentence_id: ty.Optional[str] = None
        ...
        if record_cache is None:
            __translated_obj = self.translation_handler.translate_beam_search(source_text, temperature=self.temperature_parameter)
            assert __translated_obj.log_score is not None, f"Log score is None for {source_text}"
            is_hallucination = __translated_obj.log_score < threshold
            translation_text = __translated_obj.translation_text
            log_score = __translated_obj.log_score
        else:
