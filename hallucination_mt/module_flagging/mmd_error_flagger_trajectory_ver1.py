...
class _CacheDatabaseRecordTranslation:
    ...
    translation_set: ty.Optional[ty.List[str]] = None
    is_success: bool = True

    def to_dict(self) -> ty.Dict[str, ty.Any]:
        obj = dataclasses.asdict(self)
        obj["translation_set_json"] = json.dumps(obj["translation_set"])
        obj["translation_set"] = None
        return obj

    @classmethod
    def from_dict(cls, dict_obj: ty.Dict[str, ty.Any]) -> "_CacheDatabaseRecordTranslation":
        dict_obj["translation_set"] = json.loads(dict_obj["translation_set_json"])
        return cls(**dict_obj)
...
class _CacheDatabaseRecordEmbeddingTensor:
    ...
    @staticmethod
    def bytes_to_tensor(bytes_data: bytes) -> torch.Tensor:
...
class MmdErrorFlaggerTrajectoryVer1(object):
    def __init__(self,
                 ...
                 file_name_cache_database: str = "cache_database.db",
                 max_len_a: float = 0.0,
                 max_len_b: int = 200,
        ...
        _path_cache_database = self.path_cache_dir / file_name_cache_database
        self.path_cache_database = _path_cache_database
        self.cache_database_handler = self._set_cache_database(_path_cache_database)
        # ------------------------------------------------------
    ...
    def _set_cache_database(self, path_cache_database: Path) -> module_sqlite3_handler.DBHandlerExp:
        """I set the cache database."""
        sqlite3_conn = sqlite3.connect(path_cache_database)
        ...
        module_sqlite3_handler.create_table_from_table_definition(
            ...
            primary_key="record_key_id")
        sqlite3_conn.close()
        module_logger.debug(f"Cache database is created: {path_cache_database}")
        db_handler = module_sqlite3_handler.DBHandlerExp(path_cache_database)
        return db_handler
    ...
    def _save_cache_records(self, record_obj: PossibleDataRecordType) -> None:
        if isinstance(record_obj, _CacheDatabaseRecordTranslation):
            self.cache_database_handler.insert(
    ...
    def _fetch_cache_records(self, primary_key: str, table_name: str) -> ty.Optional[PossibleDataRecordType]:
        if table_name == _CacheDatabaseRecordTranslation.__name__:
            _is_record_exist = self.cache_database_handler.is_record_exists(
                table_name=_CacheDatabaseRecordTranslation.__name__,
                exp_key=primary_key,
                is_partially_search=False,
                primary_key="record_key_id")
            if _is_record_exist:
                _obj_record_obj = self.cache_database_handler.get_record_key(
                    table_name=_CacheDatabaseRecordTranslation.__name__,
                    exp_key=primary_key,
    ...
    def flag_hallucination_one_record(self,
        ...
        # Check the cache. If the cache is available, I load the cache.
        # self.fairseq_handler.n_sampling = n_sampling
        ...
        for _tau_param in candidate_temperature_parameters:
            ...
            if _cache_record_translation is None:
                try:
                    ...
                    _seq_translated_obj = self.fairseq_handler.sample_multiple_times(
                    ...
                    _seq_translated_text = [_obj.target_text for _obj in _seq_translated_obj]
                    ...
                    _cache_db_record = _CacheDatabaseRecordTranslation(
                        ...
                        temperature=float(_tau_param),
                        source_text=eval_target.source,
                        translation_set=_seq_translated_text,
                ...
                _sampled_translation_obj = _SampledTranslationObject(
                    ...
                    translation_text=_cache_record_translation.translation_set,
        ...
        if _cache_record_embedding_y is None:
            y_emb_unfixed = extract_word_embeddings(
                model=self.model_encoder_decoder_mt,
                sentence=eval_target.target)
