...
def __rule_v1(x: np.ndarray, y_smooth: np.ndarray, threshold: float = 0.01) -> str:
...
def __rule_v2(x: np.ndarray, y_smooth: np.ndarray, spacer: float, thereshold_ratio_diff: float) -> str:
    """The rule consists of two parts,
    rule-1: the minimum y-value should be in the middle region of the x-axis.
...
def classify_function_shape(x: np.ndarray, 
    ...
    if rule_version == 'v1':
        shape = __rule_v1(x, y_smooth, threshold)
    elif rule_version == 'v2':
        shape = __rule_v2(x, y_smooth, spacer, thereshold_ratio_diff)
    else:
