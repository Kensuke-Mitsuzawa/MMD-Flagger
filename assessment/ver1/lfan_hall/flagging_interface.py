...
from hallucination_mt.baselines.seq_log_probability import FlaggerSeqLogProbability, OutputLogProbabilityFlagger
...
from hallucination_mt.module_flagging import mmd_error_flagger_trajectory_ver1
# fairseq handler
...
class InterfaceConfig:
    ...
    def __post_init__(self):
        ...
        for _key, _value in self.approach_configs.items():
            ...
            elif _key == "MmdErrorFlaggerTrajectoryVer1":
                approach_configs[_key] = dacite.from_dict(MmdErrorFlaggerTrajectoryVer1, _value)
...
def _select_non_calibration_record(seq_calibration_record: ty.List[WMTDatasetRecord],
                                   seq_dataset_record: ty.List[WMTDatasetRecord]) -> ty.List[WMTDatasetRecord]:
    seq_calibration_text_id = [__r.sentence_id for __r in seq_calibration_record]
    seq_non_calibration_record = [__record for __record in seq_dataset_record if int(__record.sentence_id) not in seq_calibration_text_id]
    return seq_non_calibration_record
# end def


def _select_calibration_text(seq_dataset_record: ty.List[WMTDatasetRecord],
                             n_calibration_records: int,
                             method_select_calibration: str = 'random',
                             seed_random_seed: int = 42,
                             ) -> ty.List[WMTDatasetRecord]:
...
def proposal_mmd_flagging_trajectory_ver1(
        config_name: str,
        algorithm_config_obj: MmdErrorFlaggerTrajectoryVer1,
        seq_dataset_eval: ty.List[WMTDatasetRecord],
        seq_calibration_record: ty.List[WMTDatasetRecord],
        management_db_handler: DBHandlerExp,
        file_logger: logging.Logger,
        path_cache_dir: Path):

    def _get_record_processed(seq_dataset_reocrd: ty.List[WMTDatasetRecord], 
                              ...
                              ) -> ty.List[mmd_error_flagger_trajectory_ver1.EvaluationTargetTranslationPair]:
        seq_record_id_existing = db_handler.get_all_keys(
            table_name=DbTableRecordProposalMmdFlaggerTrajectoryVer1.__name__,
            primary_key_field='record_id')
        
        seq_record_processing = []
        for __record in seq_dataset_reocrd:
            _db_record_id = DbTableRecordProposalMmdFlaggerTrajectoryVer1.get_record_id(
            ...
            if _db_record_id in seq_record_id_existing:
                ...
                _record_eval_input = mmd_error_flagger_trajectory_ver1.EvaluationTargetTranslationPair(
    ...
    def _execute_flagging(seq_dataset_reocrd: ty.List[WMTDatasetRecord], 
        ...
        mmd_flagger = mmd_error_flagger_trajectory_ver1.MmdErrorFlaggerTrajectoryVer1(
        ...
        _dataset_record: mmd_error_flagger_trajectory_ver1.EvaluationTargetTranslationPair
        for _dataset_record in tqdm.tqdm(seq_processing_records, 
            ...
            try:
                ...
                record_ = DbTableRecordProposalMmdFlaggerTrajectoryVer1(
                    config_name=config_name,
                ...
                db_handler.insert(table_name=DbTableRecordProposalMmdFlaggerTrajectoryVer1.__name__, data=record_dict)
                file_logger.debug(f"Done saving the result into the db.")
            # end try
        # end for
    # end def

    # loading the MT-model
    ...
    path_cache_dir = path_cache_dir / mmd_error_flagger_trajectory_ver1.__name__
    ...
    option_n_sampling = algorithm_config_obj.option_n_translation_sampling
    option_temperature_sequence = algorithm_config_obj.option_temperature_sequence
    ...
    for _n_sampling, tau_sequence, _trajectroy_rule, _trajectory_rule_smoothing, trajectory_rule_smoothing_window in option_combinations:
...
def proposal_mmd_flagging_ver1(
    ...
    def _get_record_processing(seq_dataset_reocrd: ty.List[WMTDatasetRecord], 
...
def oscillatory_detection_Raunak_2021_flagging(
    ...
    # main loop
    for _record in tqdm.tqdm(seq_dataset_eval, desc=f"Processing {raunak_2021_config_obj.approach_name}", file=sys.stdout):
    # for _record in seq_dataset_record:
        _sentence_id = _record.sentence_id
...
def seq_log_prob_Guerreiro_2023_flagging(
    ...
    path_cache_dir_algorithm = path_cache_dir / Guerreiro2023SeqLogprob.__name__
    ...
    # I initialise the flagger object.
    flagger_obj = FlaggerSeqLogProbability(
    ...
    file_logger.info(f"Computing Log-Probability over records of a dataset.")
    ...
    seq_log_prob_distribution= flagger_obj.compute_dataset_log_probability(seq_eval_records)
    threshold_flag = flagger_obj.get_flag_threshold(seq_log_prob_distribution, algorithm_config_obj.percentile_threshold)
    ...
    for _record in tqdm.tqdm(seq_dataset_eval, desc=f"Processing {algorithm_config_obj.approach_name}", file=sys.stdout):
        ...
        # I flag the record.
        _log_prob_obj = flagger_obj.flag(source_text=_record.source, threshold=threshold_flag)
...
def main(path_config_toml: Path):
    ...
    seq_dataset_records = load_dataset(path_dataset_tsv, delimiter="\t")
    ...
    # create_table_from_table_definition(db_connection, DbTableRecordProposalMmdFlaggerTrajectoryVer1, primary_key="record_id")
    ...
    # separating the calibration records
    seq_calibration_record = _select_calibration_text(
        seq_dataset_record=seq_dataset_records,
        n_calibration_records=interface_config.resource_config.n_calibration_records,
        method_select_calibration=interface_config.resource_config.method_select_calibration,
        seed_random_seed=interface_config.resource_config.seed_random_seed)
    seq_dataset_eval = _select_non_calibration_record(
        seq_calibration_record=seq_calibration_record,
        seq_dataset_record=seq_dataset_records)

    if interface_config.resource_config.limit_dataset_record is not None:
        seq_dataset_eval = seq_dataset_eval[:interface_config.resource_config.limit_dataset_record]
    # end if
    ...
    for _approach_name in seq_approach_names:
        if _approach_name == "MmdErrorFlaggerVer1":
            raise NotImplementedError("This implementation is out-dated already.")
        ...
        elif _approach_name == "MmdErrorFlaggerTrajectoryVer1":
            raise NotImplementedError("This implementation is out-dated already.")            
            algorithm_config = interface_config.approach_configs["MmdErrorFlaggerTrajectoryVer1"]
            assert isinstance(algorithm_config, MmdErrorFlaggerTrajectoryVer1), f"Expected MmdErrorFlaggerTrajectoryVer1, got {algorithm_config}"
            proposal_mmd_flagging_trajectory_ver1(
                config_name=interface_config.config_name,
...
def evaluate_main(path_config_toml: Path):
    ...
    seq_dataset_record = load_dataset(interface_config.resource_config.path_dataset_tsv, delimiter="\t")
    ...
    seq_eval_table_name = [
        ...
        # DbTableRecordProposalMmdFlaggerTrajectoryVer1.__name__,
