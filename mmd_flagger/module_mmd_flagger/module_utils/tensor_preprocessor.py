"""A module to aggregate a document (N-length-tokens) into a fixed embedding."""
...
MODE_VECTOR_PREPROCESS = ty.Literal['avg', 'concat']
MODE_MAX_TOKEN_LENGTH_VECTOR_CONCAT = ty.Literal['max_calibration', 'fixed', None]


class BaseTensorPreprocessor(abc.ABC):
    @abc.abstractmethod
...
class TensorPreprocessorWordEmbeddings(BaseTensorPreprocessor):
    def __init__(
            self,
            mode_vector_preprocess: MODE_VECTOR_PREPROCESS = "avg",
            mode_max_token_length_vector_concat: MODE_MAX_TOKEN_LENGTH_VECTOR_CONCAT = "max_calibration",
    ...
    def preprocess_tensors(self,
        ...
        if self.mode_vector_preprocess == "avg":
            ...
            if isinstance(self.mode_max_token_length_vector_concat, int):  # fixed token size.
                ...
                raise ValueError(f'No name option: {self.mode_max_token_length_vector_concat}')
            # end if
        else:
            raise ValueError(f"Invalid mode preprocess: {self.mode_vector_preprocess}")


# The following name has been used for the older versions.
TensorPreprocessorVer1 = TensorPreprocessorWordEmbeddings

# ----

class TensorPreprocessorReshape(BaseTensorPreprocessor):
    def preprocess_tensors(self,
                           seq_tensor: ty.List[torch.Tensor],
                           is_calibration_mode: bool = False
                           ) -> torch.Tensor:
        """Reshaping [Tensor] (N-dim) -> Tensor (N-doc, N-dim), where N-dim are consistent.
        """
        return torch.stack(seq_tensor)
# end class
